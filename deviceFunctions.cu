...
__device__ bool validCell(const int row, const int col) {
    if (row < 0 || row >= TOTAL_CELLS || col < 0 || col >= TOTAL_CELLS) return false;
    return true;
}

__global__ void calculateCellTemp(float *cellsIn, float *cellsOut) {
    unsigned int col = blockIdx.x * blockDim.x + threadIdx.x;
    unsigned int row = blockIdx.y * blockDim.y + threadIdx.y;
    int globalIndex = static_cast<int>(row) * CELL_SIZE_W + static_cast<int>(col);

    if (validCell(static_cast<int>(row), static_cast<int>(col)) == false) return;
    ...
    for (auto &direction : directions) {
        ...
        if (validCell(x, y) == false) continue;
